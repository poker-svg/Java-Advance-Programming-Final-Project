...
public class AsciiPanel extends JPanel {
    ...
    public static Color grassGreen = new Color(56, 217, 115);

    public static Color brown = new Color(191, 121, 88);
    ...
    public void setAsciiFont(AsciiFont font) {
        ...
        glyphs = new BufferedImage[256+font.rowNum()*font.columnNum()];
     ...
     * @param width
     * @param height
     * @param font   if passing null, standard font CP437_16x16 will be used
     */
    ...
    private void loadGlyphs() {
        try {
            glyphSprite = ImageIO.read(AsciiPanel.class.getClassLoader().getResource(AsciiFont.CP437_16x16.getFontFilename()));
        } catch (IOException e) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            glyphs[i] = new BufferedImage(16, 16, BufferedImage.TYPE_INT_ARGB);
            glyphs[i].getGraphics().drawImage(glyphSprite, 0, 0, 16, 16, sx, sy, sx + 16,
                    sy + 16, null);
        }
        ...
        for (int i = 0; i < this.asciiFont.rowNum()*this.asciiFont.columnNum(); i++) {
            int sx = (i % this.asciiFont.columnNum()) * charWidth;
            int sy = (i / this.asciiFont.columnNum()) * charHeight;

            glyphs[256+i] = new BufferedImage(charWidth, charHeight, BufferedImage.TYPE_INT_ARGB);
            glyphs[256+i].getGraphics().drawImage(glyphSprite, 0, 0, charWidth, charHeight, sx, sy, sx + charWidth,
                    sy + charHeight, null);
        }
    }
